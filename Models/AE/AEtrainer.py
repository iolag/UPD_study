...
from Models.AE.AEmodel import AE
...
def ae_train_step(model, optimizer, input) -> dict:
    model.train()
    optimizer.zero_grad()
    input_recon = model(input.repeat(1, 1, 1, 1))
    loss_dict = model.loss_function(input, input_recon)  # VAE loss
    loss = loss_dict['loss']
    loss.backward()
    optimizer.step()
    return loss_dict
...
def ae_val_step(model, input, return_loss: bool = True) -> Tuple[dict, Tensor]:

    model.eval()
    with torch.no_grad():
        input_recon = model(input.repeat(1, 1, 1, 1))
    loss_dict = model.loss_function(input, input_recon)  # AE Loss
...
def validate(model, val_loader, i_iter, config) -> None:
    ...
    for input in val_loader:
        ...
        loss_dict, anomaly_map, _, input_recon = ae_val_step(model, input)
...
def train(model):
    ...
    while True:
        for input in train_loader:
            ...
            # Train step
            loss_dict = ae_train_step(model, optimizer, input)
            ...
            if i_iter % config.anom_val_frequency == 0:
                eval_reconstruction_based(model, small_testloader, i_iter, ae_val_step, logger, config)
...
if __name__ == '__main__':
    if config.eval:
        ...
        eval_reconstruction_based(model, big_testloader, 0, ae_val_step, logger, config)
