...
def sample_location(img: np.ndarray, core_percent: float = 0.4, is_mri: bool = False) -> np.ndarray:
    """
    ...
    """
    while True:  # TODO check if img is all zeros
...
def sample_width(img: np.ndarray, min_p: float = 0.05, max_p: float = 0.3) -> float:
    """
...
def create_patch_mask(img: np.ndarray, is_mri: bool = False) -> np.ndarray:
    ...
    mask[..., coor_min1[0]:coor_max1[0], coor_min1[1]:coor_max1[1]] = 1
